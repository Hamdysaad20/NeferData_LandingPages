...
const Products = ({ products, bg }) => {
  ...
  return (
    <section
      className={`rounded-[40px] m-auto my-12 mt-0 p-4 w-11/12 lg:flex-row flex flex-col gap-4 min-h-[500px] lg:h-[700px] max-w-screen-2xl ${bg}`}
    ...
    >
      <div className={productAreaClass}>
        {products.slice(0, productCount).map((product, index) => (
          <button
            className={`flex justify-start w-full items-start border-t-2 border-gray-100/20 p-4 text-start text-white overflow-hidden transition-all duration-500 rounded-xl before:aspect-square before:translate-x-1 before:translate-y-1/2 before:rounded-full before:bg-white before:absolute before:bottom-0 after:w-3/4 after:bg-gradient-to-r after:from-transparent after:to-white after:bottom-0 after:absolute relative ${activeProductIndex === index
          ...
          >
            ...
            <div
              className={`flex flex-col relative justify-start items-start transition-all duration-500 overflow-hidden ${activeProductIndex === index ? "h-30 pb-12" : "h-8"
                }`}
            >
            ...
            <Link
              to="/pricing"
              className="text-white absolute inline-block bottom-4 right-4 hover:bg-black/40 duration-500 w-fit select-none bg-black/20 p-2 px-4 rounded-full text-sm font-semibold mt-2 cursor-pointer"
              style={{
