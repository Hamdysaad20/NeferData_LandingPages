...
const Features = ({ tabs }) => {
  ...
  return (
   ...
   }} className='w-full justify-center pb-12 pt-32 flex'>
   <div
              style={{
                fontFamily:
              ...
              <div className='flex flex-col md:flex-row place-items-center'>
                <span>Neferdata is a suite of <span className='underline decoration-sky-500'>AI-powered tools</span> to help grow your business</span>
              </div>
      ...
      <div className='flex gap-2  rounded-3xl  overflow-hidden overflow-x-auto scroll-ml-6 md:justify-center  w-full px-4'>
        {tabs.map((tab, index) => (
          <button
            ...
            className={`flex select-none flex-col  hover:opacity-75 duration-500 active:scale-95 justify-start text-center items-center  rounded-3xl p-4  w-48  gap-3 font-semibold transition-all text-blue-1 ${activeTab === index ? 'opacity-100 hover:opacity-100' : 'opacity-50 hover:opacity-80'}`}
      ...
      <div className='flex flex-col h-[700px] bg-[#3092DB] backdrop-blur-2xl w-full rounded-2xl p-4 divide-y-2'>
        <div className='flex overflow-hidden overflow-x-auto relative gap-4 pb-4'>
          {tabs[activeTab].features.map((feature, index) => (
            <button
              key={index}
              className={`grid ease-linear border-2 active:scale-95 duration-500 select-none place-content-center grid-cols-6 box-content relative h-12 w-54 text-center items-center rounded-2xl p-1 gap-3 font-semibold transition-all text-black ${activeFeature === index ? 'opacity-100 hover:opacity-100 bg-gray-200 border border-black' : 'opacity-50 border-white/20 hover:opacity-80'}`}
            ...
            >
              ...
              <div className='grid col-span-5 place-items-center text-sm break-keep box-content ' style={{ width: 'calc(100% - 15px)' }}>{feature.title}</div>
            </button>
          ))}
        </div>
