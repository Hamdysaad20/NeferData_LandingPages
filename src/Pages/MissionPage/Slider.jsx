...
function Slider() {
  ...
  const FoundersNotes = [
    ...
    },
    {
      ...
      classes: "A Unique Edge in SaaS Transformation",
      companyLogo: NeferData,
    },
  ];
      ...
      <AnimatePresence mode='wait'>
        <div className='flex flex-col gap-6 text-white text-center box-border px-0 lg:px-16 max-lg:px-2 max-md:px-0 max-w-[890px]'>
          ...
          <motion.p
            ...
            <span className="text-black text-extrabold">&quot; </span>{currentFoundersNotes.message}{" "} <span className="text-black text-extrabold">&quot;</span>
          </motion.p>
        </div>
        <div className='absolute bottom-5  gap-4 left-0 right-0 text-center text-white flex justify-center'>
          <div className={`w-12 ring-black cursor-pointer select-none relative h-12 rounded-full overflow-hidden bg-white ${currentFoundersNotes === FoundersNotes[1] ? 'opacity-100 ring-2  animate-pulse duration-1000' : 'opacity-20'}`}>
            <img
              src={Kamil_Litman}
              alt=''
          ...
          <div className={`relative  ring-black cursor-pointer select-none w-12 h-12 rounded-full overflow-hidden bg-white ${currentFoundersNotes === FoundersNotes[0] ? 'opacity-100 ring-2  animate-pulse duration-1000' : 'opacity-20'}`}>
            <img
              src={Dario_Lencina}
              alt=''
              className='w-full h-full object-cover object-center'
